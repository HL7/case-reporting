...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Parameters/f:parameter">
      ...
      <sch:assert test="exists(f:value) or exists(f:resource) and not(exists(f:value) and exists(f:resource))">A parameter must have only one of (value, resource, part) (inherited)</sch:assert>
      <sch:assert test="@value|f:*|h:div">All FHIR elements must have a @value or children (inherited)</sch:assert>
      <sch:assert test="exists(f:value) or exists(f:resource) and not(exists(f:value) and exists(f:resource))">A parameter must have only one of (value, resource, part) (inherited)</sch:assert>
      <sch:assert test="@value|f:*|h:div">All FHIR elements must have a @value or children (inherited)</sch:assert>
      <sch:assert test="exists(f:value) or exists(f:resource) and not(exists(f:value) and exists(f:resource))">A parameter must have only one of (value, resource, part) (inherited)</sch:assert>
