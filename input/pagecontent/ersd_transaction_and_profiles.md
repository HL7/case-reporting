...
**Expansion parameters are carried on the manifest.** The manifest carries the `cqf-expansionParameters` and `cqf-inputParameters` extensions, which reference a contained `Parameters` resource pinning the version of each code system used when the value sets in the package were expanded, together with the expansion settings applied. This matters to any implementation that re-expands the value sets rather than using the expansions as distributed: expanding against different code system versions, or with different settings, can produce a different set of codes and therefore different triggering behaviour. An implementation that re-expands should use these parameters.
